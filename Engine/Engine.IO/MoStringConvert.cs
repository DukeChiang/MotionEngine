...
namespace MotionEngine.IO
{
	public static class MoStringConvert
	{
		...
		public static T IndexToEnum<T>(int index) where T : IConvertible
		{
			...
			{
				string message = string.Format("Enum {0} is not defined index {1}", typeof(T), index);
		...
		public static T NameToEnum<T>(string name)
		{
			...
			{
				string message = string.Format("Enum {0} is not defined name {1}", typeof(T), name);
				throw new ArgumentException(message);
			}
		...
		/// 字符串转换为参数列表
		/// </summary>
		public static List<float> StringToCodeParams(string str)
		{
